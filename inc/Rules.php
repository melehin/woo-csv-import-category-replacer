...
class Rules {
    ...
    public function get_rules( $per_page = 5, $page_number = 1 ) {
        ...
        $sql = $wpdb->prepare( "SELECT * FROM {$this->table_name}  ORDER BY `name` DESC, `from` DESC LIMIT %d OFFSET %d", $per_page, ( $page_number - 1 ) * $per_page );
     ...
     */
    public function add_rule( $from, $name, $to ) {
        global $wpdb;
...
class RulesTable extends \WP_List_Table {
    ...
    public static function admin_header() {
        ...
        echo '.wp-list-table .column-from { width: 33%; }';
        echo '.wp-list-table .column-name { width: 33%; }';
        echo '.wp-list-table .column-to { width: 33%; }';
    ...
    public function process_bulk_action() {
        ...
        if ( isset( $_POST['action'] ) && $_POST['action'] == 'add' ) {
          $this->rules->add_rule( esc_sql( $_POST['from'] ), esc_sql( $_POST['name'] ), esc_sql( $_POST['to'] ) );
